...
export function migrate() {
    ...
    const get = db.prepare('SELECT max(version) as v FROM schema_version').get();
    ...
    if (applied)
        console.log(`[api] migrations applied: ${applied}`);
}
...
export function seedMinimal() {
    ...
    const makeId = (slug) => slug + ':' + ulid().slice(0, 6);
