...
export const TransformsPanel = memo(function TransformsPanel({
  ...
  return (
    <div className="space-y-3 text-sm">
      {!readOnly && <div className="text-xs text-muted-foreground">Toggle identity transforms and set parameters.</div>}
          ...
          <div key={family} className="space-y-2">
            ...
            {transforms.map((t) => (
              <div key={t.id} className="rounded border p-2">
                ...
                {t.schema && t.schema.length > 0 && (readOnly || isChosen(t.id)) && (
                      ...
                      <div key={p.key} className="space-y-1">
                        ...
                        {readOnly ? (
                          <div className="text-xs text-muted-foreground">
                            {String(currentParams(t.id)[p.key] ?? '—')}
                          </div>
                        ) : (
                          <>
                            {p.kind === 'boolean' && (
