...
* **Minimum Segments**: 2 (e.g., `tx:p`, `tx:a`)
* **Maximum Segments**: 4 (kingdom + genus + species + cultivar/breed)
* **Full ID**: No trailing colon (e.g., `tx:p:lactuca:sativa:romaine`)
* **Parent**: Computed by dropping last segment (e.g., `tx:p:lactuca:sativa:romaine` → parent = `tx:p:lactuca:sativa`)
* **NCBI Integration**: `ncbi_taxid` field stores NCBI taxonomy ID for species/genus level verification
* **Examples**:
  - `tx:p` (plants kingdom)
  - `tx:p:lactuca:sativa:romaine` (cultivar)
  - `tx:a:bos:taurus` (species)
  - `tx:f:agaricus:bisporus` (species)
...
### 1.3) Rank Structure (NCBI-integrated)

We use NCBI taxonomy as the authoritative source for biological classification, with our ontology focusing on food-relevant taxa at genus, species, and below-species levels.

**Kingdom-specific patterns:**
- **Plants**: `tx:p:{genus}:{species}[:{cultivar}]`
- **Animals**: `tx:a:{genus}:{species}[:{breed}]`  
- **Fungi**: `tx:f:{genus}:{species}`

**NCBI Integration:**
- `ncbi_taxid` field stores NCBI taxonomy ID for species/genus level verification
- Full lineage (kingdom, phylum, class, order, family) computed from NCBI during build
- Applicability rules use NCBI lineage for matching (e.g., `tx:a:chordata` matches any taxon with phylum=Chordata)

**Below-species ranks:**
- Cultivars (plants): `tx:p:lactuca:sativa:romaine`
- Varieties (plants): `tx:p:brassica:oleracea:italica`
- Breeds (animals): `tx:a:bos:taurus:holstein`

**Parent relationships:** Computed from ID structure by dropping the last segment.
...
```json
{
  "id":"tpt:tx:a:bos:taurus:part:milk:unknown:greek-yogurt",
  "taxon_id":"tx:a:bos:taurus",
  "part_id":"part:milk",
  "family":"DAIRY_YOGURT",
...
```json
{
  "id":"tpt:tx:a:bos:taurus:part:butter:unknown:ghee",
  "taxon_id":"tx:a:bos:taurus",
  "part_id":"part:butter",
  "family":"DAIRY_BUTTER",
...
```json
{
  "id":"tpt:tx:a:sus:scrofa_domesticus:part:belly:unknown:us-bacon",
  "taxon_id":"tx:a:sus:scrofa_domesticus",
  "part_id":"part:belly",
...
```json
{
  "id":"tpt:tx:a:salmo:salar:part:fillet:unknown:cold-smoked-salmon",
  "taxon_id":"tx:a:salmo:salar",
  "part_id":"part:fillet",
...
```json
{
  "id":"tpt:tx:a:gadus:morhua:part:fillet:unknown:bacalhau",
  "taxon_id":"tx:a:gadus:morhua",
  "part_id":"part:fillet",
