...
* **Taxa (`tx:*`)**: `tx:{kingdom}:{genus}:{species}[:variety]`. Kingdom prefixes: `a:` (animalia), `p:` (plantae), `f:` (fungi).
  * **ACTUAL FORMAT**: `tx:a:bos:taurus` (includes genus), `tx:p:lactuca:sativa:capitata` (includes variety)
  * **Required fields**: `id`, `rank`, `display_name`, `latin_name`
  * **Optional fields**: `aliases[]`, `ncbi_taxid`, `parent`
...
* **TPTs**: `{taxon_id}|{part_id}|{identity_hash}` where identity_hash is first 12 chars of SHA1 of normalized identity transforms
  * **NOT**: `tpt:tx:full:lineage:part:name:family:variant`
  * **Family is separate field**, not part of ID generation
...
* **`parts.core.jsonl`**: biological/anatomical + primary process parts (e.g., milk, cream, muscle, fruit).
...
* **`compiled/parts.registry.json`**: merged registry (generated by Stage 0)
...
**Core parts** (`parts.core.jsonl`): Biological/anatomical + primary process parts (e.g., milk, cream, muscle, fruit).
**Derived parts** (`parts.derived.jsonl`): Product parts (butter, yogurt, hard cheese, lard, ghee, oil:virgin/refined, fillet, fruit:peel:*).
**Registry merge**: Stage 0 merges these into `compiled/parts.registry.json` (the single authoritative parts table used by all later stages).
...
`parts.core.jsonl` holds primary/anatomical parts. Must NOT contain `kind:"derived"`.
...
The build step emits `compiled/parts.registry.json` which downstream stages consume as if it were a single `parts.json`.
...
7. **Core purity**: `parts.core.jsonl` must not contain `kind:"derived"`.

### Contributor workflow

- Add/modify anatomical parts in `parts.core.jsonl`.
...
- Run the build to generate `compiled/parts.registry.json`; review validation reports.

### Part Ancestry

`part_ancestors` is built from `part_def` loaded from `compiled/parts.registry.json`.
...
Confirm that `part:cream` exists in `parts.core.jsonl` and that the merged registry validates.
...
```json
{
  "id":"tx:a:bos:taurus|part:milk|a1b2c3d4e5f6",
  "taxon_id":"tx:a:bos:taurus",
  "part_id":"part:milk",
  "family":"CULTURED_DAIRY",
  "transforms":[{"id":"tf:ferment","params":{"starter":"yogurt_thermo"}},{"id":"tf:strain","params":{"strain_level":6}}],
  "name":"Greek Yogurt",
...
```json
{
  "id":"tx:a:bos:taurus|part:butter|b2c3d4e5f6g7",
  "taxon_id":"tx:a:bos:taurus",
  "part_id":"part:butter",
  "family":"BUTTER_DERIVATIVES",
  "transforms":[{"id":"tf:clarify_butter","params":{"stage":"ghee"}}],
  "name":"Ghee",
...
```json
{
  "id":"tx:a:sus:scrofa_domesticus|part:belly|c3d4e5f6g7h8",
...
```json
{
  "id":"tx:a:salmo:salar|part:fillet|d4e5f6g7h8i9",
  "taxon_id":"tx:a:salmo:salar",
  "part_id":"part:fillet",
  "family":"SMOKED_MEAT_FISH",
  "transforms":[
...
```json
{
  "id":"tx:a:gadus:morrhua|part:fillet|e5f6g7h8i9j0",
  "taxon_id":"tx:a:gadus:morrhua",
  "part_id":"part:fillet",
