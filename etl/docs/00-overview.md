...
The ETL (Extract, Transform, Load) pipeline compiles the Food Graph from raw ontology data and external evidence sources into a comprehensive knowledge graph database. The pipeline follows the TPT vision (Taxon, Taxon+Part, Taxon+Part+Transform) with an 8-stage compiler.
...
5. **Composable**: easy to add new stages without touching others
